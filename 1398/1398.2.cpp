...
//确定出口位置
	...
	//****test
	//	cout<<start[0][0]<<" "<<start[0][1]<<endl<<start[1][0]<<" "<<start[1][1]<<endl;
	//*******
		...
		for(i=1;!(bq1&&bq2);++i)
		{
			...
			{//cout<<"1111111111111"<<endl;
				...
				while(a.num==i)
				{
					//cout<<"CUR:"<<a.y<<","<<a.x<<endl;
					for(j=0;j<4;++j)
					{
						if(pass(a.y,a.x,j))
						{
							//cout<<"PASS:"<<a.y<<","<<a.x<<":"<<j<<endl;
							switch(j)
							{
							case 0:
								if(!steped[a.y-2][a.x]){//cout<<"↑"<<endl;
								q1.push(gezi(a.y-2,a.x,i+1));steped[a.y-2][a.x]=1;}break;
							case 1:
								if(!steped[a.y+2][a.x]){//cout<<"↓"<<endl;
								q1.push(gezi(a.y+2,a.x,i+1));steped[a.y+2][a.x]=1;}break;
							case 2:
								if(!steped[a.y][a.x-2]){//cout<<"←"<<endl;

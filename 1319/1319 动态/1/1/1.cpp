...
int step(int cityid,int stepnum){
    int t;
	if(dongtai[cityid]!=0) {
		if(dongtai[cityid]!=-1&&minstep>stepnum+dongtai[cityid])
		minstep=stepnum+dongtai[cityid];
		return dongtai[cityid];
	                        }
     ...
     for(j=0;j<cityinstant[cityid].linenum;j++){
       ...
       //        printf(".......");
         ...
         for(i=0;i<stepnum;i++){
          ...
          if(!reached){
             t=step(cityinstant[cityid].line[j],stepnum+1); 
			 if(t==-1) ;
             else if(t+1<dongtai[cityid]) dongtai[cityid]=t+1;
		                }
          
        
        
                                                }
	   if(dongtai[cityid]==0) return -1;
	   else return dongtai[cityid];
     }
